...
function Marquee({ activeTheme }) {
  ...
  return (
    <div className={`relative z-30 w-full overflow-hidden py-1.5 shadow-sm select-none border-b border-black/10 transition-colors duration-700 ${bgStyle}`}>
        ...
        {[0, 1].map((half) => (
          <div key={half} className="flex shrink-0 items-center">
            {items.map((_, i) => (
              <span
                key={i}
                className="mx-4 flex items-center gap-3 font-geometric text-[11px] font-black uppercase tracking-[0.24em]"
              >
  ...
  return (
    ...
    >
      <Image
        ...
        className="h-8 sm:h-9 md:h-10 w-auto max-w-[200px] sm:max-w-[240px] md:max-w-[280px] object-contain brightness-110 drop-shadow-[0_2px_12px_rgba(255,255,255,0.2)]"
      />
    </div>
  );
}
...
export default function HeroSection() {
  ...
  return (
    <div
      ref={heroRef}
      className={`relative flex h-screen w-full flex-col overflow-hidden select-none transition-colors duration-700 ${activeVariation.bgClass}`}
    >
      {/* 1. TOP MARQUEE WITH DYNAMIC THEMED ACCENTS */}
      <Marquee activeTheme={activeVariation.themeId} />

      {/* 2. TOP DARK NAVBAR (#000000 BLACK) */}
      <header className="relative z-30 flex h-16 md:h-18 w-full items-center justify-between bg-[#000000] px-4 md:px-8 text-white backdrop-blur-xl border-b border-white/10 shadow-lg font-geometric">
        ...
        <div className="flex items-center gap-3.5 sm:gap-4 md:gap-5 text-[#EFEEE8]">
          <MagneticElement strength={10}>
            <button
              onClick={cycleCurrency}
              title={`Switch currency (Current: ${activeCurrency.code})`}
              className="flex h-9 min-w-[36px] items-center justify-center rounded-xl border border-white/10 bg-white/5 backdrop-blur-md px-2.5 font-geometric text-xs font-bold text-white transition-all hover:border-[#EF0606] hover:bg-white/10 hover:text-[#EF0606] shadow-sm"
            >
      ...
      <section className="relative flex-1 overflow-hidden">
        ...
        {/* 3D Scene Canvas with Active Themed Model */}
        ...
        {activeVariation.themeId === "noir" && (
          <>
            {/* Left Headline: Asymmetric Didone Editorial Typography */}
        ...
        {activeVariation.themeId === "noir" && (
          <div className="pointer-events-none absolute inset-x-0 bottom-0 z-[15] h-7 w-full overflow-hidden transition-opacity duration-500">
            <svg
